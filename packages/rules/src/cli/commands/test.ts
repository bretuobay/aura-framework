...
export const testCommand = new Command("test")
  ...
  .action(async (fixtureGlob: string, options: { verbose?: boolean; rules?: string }) => {
    // 1. Resolve fixture files via glob
    const filePaths = await fg(fixtureGlob);

    // Req 13.4: No fixtures match → warning + exit 0
    if (filePaths.length === 0) {
      console.log(`Warning: No fixture files matched glob "${fixtureGlob}"`);
      process.exit(0);
    }
    ...
    for (const filePath of filePaths) {
      try {
        ...
        if (fixtures.length === 0) {
          ...
          console.error(`Error: Fixture file cannot be parsed: ${filePath}`);
          process.exit(1);
        }
        ...
        const message = err instanceof Error ? err.message : String(err);
        console.error(`Error: Failed to parse fixture file "${filePath}": ${message}`);
        process.exit(1);
      }
    }
    ...
    if (options.verbose) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        console.log(`${statusIcon} [${statusLabel}] ${fixture.id} — ${fixture.description}`);
